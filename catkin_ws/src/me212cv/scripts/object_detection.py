...
#Publisher for publishing object position
obj_pub = rospy.Publisher('/object_position', Point, queue_size=10)
...
def HSVObjectDetection(cv_image, toPrint = True):
    ...
    lower_red = np.array([140,70,140])
    upper_red = np.array([180,160,220])
...
def rosRGBDCallBack(rgb_data, depth_data):
    ...
    for cnt in contours:
        ...
        if not math.isnan(cv_depthimage2[int(yp)][int(xp)]) and cv_depthimage2[int(yp)][int(xp)] > 0.1 and cv_depthimage2[int(yp)][int(xp)] < 10.0:
            ...
            print 'xp', xp, 'yp', yp, 'zc', zc
            obj_pos = Point(xp, yp, zc)
            obj_pub.publish(obj_pos)
        else:
            continue
